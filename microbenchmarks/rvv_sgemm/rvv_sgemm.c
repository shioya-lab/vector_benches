...
void sgemm_vec(size_t size_m, size_t size_n, size_t size_k,
  ...
  for (size_t m = 0; m < size_m; ++m) {
    ...
    for (size_t c_n_count = size_n; c_n_count; c_n_count -= vl) {
      vl = __riscv_vsetvl_e32m8(c_n_count);
      const float *a_k_ptr = a;
      const float *b_k_ptr = b_n_ptr;
      vfloat32m8_t acc = __riscv_vle32_v_f32m8(c_n_ptr, vl);
      for (size_t k = 0; k < size_k; ++k) {
        vfloat32m8_t b_n_data = __riscv_vle32_v_f32m8(b_k_ptr, vl);
        acc = __riscv_vfmacc_vf_f32m8(acc, *a_k_ptr, b_n_data, vl);
        b_k_ptr += ldb;
        a_k_ptr++;
      }
      __riscv_vse32_v_f32m8(c_n_ptr, acc, vl);
      c_n_ptr += vl;
      b_n_ptr += vl;
    }
    a += lda;
