...
def generate_sniper_output_xml(src_file, dst_file):
    ...
    for v in values:
        ...
        corename = "core-%d" % v[2]
        prefixname = "@" + prefix_map[v[0]]["prefixname"]   # @ は xml_parser の仕様
...
def main():
    try:
        ...
        # McPAT バイナリの位置などの情報を cfg.xml から取得
        # config = Config("cfg.xml")

        # sqlite から xml に stats の内容を展開
        if (True):
            print("generate_sniper_output_xml ...\t", end="")
            generate_sniper_output_xml(stats_sqlite_file, stats_xml_file)
            print("[OK]")

        # stats の xml の情報を mcpat 用の template に埋め込む
        # if (True):
        #     print("setup_mcpat ...\t", end="")
        ...
        #     print("[OK]")
        #
        # # mcpat の実行結果をパースして csv に出力
        # print("parse_mcpat_result ...\t", end="")
